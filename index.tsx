  ...
  return {
    auth: {
      signUp: async ({ email, password, options }) => {
        // Simulate sign up
        const user = { id: Date.now().toString(), email, ...options?.data };
    ...
    from: (table) => ({
      ...
      insert: (values) => ({
        select: (columns) => ({
          async execute() {
            const newRecord = { id: Date.now().toString(), ...values, created_at: new Date().toISOString() };
      ...
      update: (values) => ({
        eq: (column, value) => ({
          async execute() {
            return { data: [{ ...values, id: value }], error: null };
          }
        })
      }),
      delete: () => ({
...
const getMockData = (table, filter = {}) => {
  const mockData = {
    users: [
      { id: '1', name: 'John Doe', email: 'john@example.com', created_at: '2025-06-01T10:00:00Z' },
      { id: '2', name: 'Jane Smith', email: 'jane@example.com', created_at: '2025-06-01T11:00:00Z' },
      { id: '3', name: 'Bob Johnson', email: 'bob@example.com', created_at: '2025-06-01T12:00:00Z' },
    ],
    assignments: [
      {
        id: '1',
    ...
    assignment_completions: [
      { id: '1', assignment_id: '1', user_id: '2', completed_at: '2025-06-02T14:30:00Z' }
    ]
  };
